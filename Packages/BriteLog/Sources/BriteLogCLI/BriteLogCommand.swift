...
extension BriteLogCommand {
    ...
    struct Doctor: ParsableCommand {
        ...
        static func output(
            ...
            lines.append("  - If `local-store` is unavailable here, a signed app host may still need the right provisioning profile before macOS will let it launch with that entitlement.")
            return lines.joined(separator: "\n")
        }
